...
function initializeStatisticsStart(){
  const db=readDb();
  const saved=Date.parse(String(db.statisticsStartAt||''));
  if(Number.isFinite(saved))return;
  db.statisticsStartAt=new Date(nowMs()).toISOString();
  writeDb(db);
  console.log(`[statistics] collection started at ${db.statisticsStartAt}`);
}
function statisticsStartMs(db){
  const saved=Date.parse(String(db.statisticsStartAt||''));
  return Number.isFinite(saved)?saved:nowMs();
}
function statisticsDateTime(ms){
  if(!Number.isFinite(Number(ms)))return '—';
  return new Intl.DateTimeFormat('uk-UA',{timeZone:'Europe/Warsaw',day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'}).format(new Date(Number(ms)));
}
...
function statisticsRange(url,startMs){
  const raw=String(url.searchParams.get('period')||'30');
  const period=STATISTICS_PERIODS.has(raw)?raw:'30';
  const now=nowMs(),day=86400000;
  let requestedFrom=now-30*day,requestedTo=now,fromKey='',toKey='';
  ...
  else if(period==='all')requestedFrom=startMs;
  ...
  const partial=requestedFrom<startMs;
  const from=Math.max(requestedFrom,startMs),to=requestedTo<startMs?startMs-1:requestedTo;
  const duration=Math.max(1,requestedTo-requestedFrom+1),compareTo=requestedFrom-1,compareFrom=compareTo-duration+1;
  const comparisonAvailable=compareFrom>=startMs&&to>=from;
  return {period,from,to,requestedFrom,requestedTo,compareFrom,compareTo,comparisonAvailable,partial,fromKey,toKey};
}
...
function statisticsAggregate(db,orders,products){
  ...
  for(const order of orders){
    ...
    for(const item of Array.isArray(order.items)?order.items:[]){
      ...
      if(!row)continue;
      const quantity=Number(item.orderedQty!==undefined?item.orderedQty:item.qty);
      const total=productFormatValue(itemWithQuantityFields(item))*quantity;
      if(Number.isFinite(total)&&total>0)row.quantity=Math.round((row.quantity+total)*1000)/1000;
...
function statisticsBuild(db,url){
  const startMs=statisticsStartMs(db),range=statisticsRange(url,startMs),products=(db.products||[]).slice();
  ...
  const eligible=(db.orders||[]).filter(o=>{const ms=statisticsOrderMs(o);return ms>=startMs&&(!shop||String(o.shop||'')===shop);});
...
function statisticsPage(db,url){
  ...
  const periods=[['7','Останні 7 днів'],['30','Останні 30 днів'],['60','Останні 60 днів'],['90','Останні 90 днів'],['current_month','Поточний місяць'],['previous_month','Попередній місяць'],['custom','Власний період'],['all','Від початку статистики']];
  ...
  const cards=[['Усього товарів',data.totalProducts,'📦'],['Замовляли',data.orderedProducts,'✓'],['Ще не замовляли',data.neverProducts,'○'],['Нових замовлень',data.orderCount,'🧾'],['Активних магазинів',data.activeShopCount,'🏪'],['Попит зростає',data.growthCount,'↗'],['Попит знижується',data.declineCount,'↘']];
  const topHtml=top.length?top.map((r,i)=>`<div class="statsTopRow"><span class="statsRank">${i+1}</span><div class="statsTopInfo"><div><b>${esc(r.name)}</b><span>${r.orderCount} замовл. · ${fmtNum(r.quantity)} ${esc(r.unit)} · ${r.shops.size} магаз.</span></div><small>${esc(r.category)} · останнє: ${esc(statisticsDate(r.lastMs))}</small><div class="statsBar"><i style="width:${Math.max(3,Math.round(rankValue(r)/maxTop*100))}%"></i></div></div></div>`).join(''):'<div class="statsEmpty">Нових замовлень ще немає.</div>';
  ...
  const notice=data.range.partial?`<div class="statsNotice">Дані доступні лише з ${esc(statisticsDateTime(data.startMs))}. Частина вибраного періоду перед початком збору статистики не враховується.</div>`:'';
  const compareNotice=!data.range.comparisonAvailable?'<div class="statsNotice statsNoticeNeutral">Ще недостатньо даних для порівняння з попереднім періодом.</div>':'';
...
<script>(function(){
  ...
  if(req.method==='GET' && url.pathname==='/admin-statistics'){ if(!requireAdmin(req,res,session)) return; return send(res,layout('Статистика',statisticsPage(db,url),session)); }
