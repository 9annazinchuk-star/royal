...
function marketCalc(record,sellerId,productId){
  ...
  const accepted=hasAccepted?Number(row.accepted):0,returned=hasReturned?Number(row.returned):0,sold=Math.max(0,accepted-returned);
...
function marketGrandTotal(record){return (record&&record.sellerSnapshots||[]).reduce((sum,s)=>sum+marketSellerTotal(record,s.id),0);}
...
.marketPage{max-width:1280px;min-width:0}.marketTop,.marketToolbar,.marketTemplateHead,.marketHistoryHead{display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap}.marketTop{margin-bottom:14px}.marketTop h1,.marketHistoryHead h1{margin:0}.marketTopActions,.marketPrintActions,.marketTabs{display:flex;gap:8px;align-items:center;flex-wrap:wrap}.marketTabs{margin:12px 0}.marketTabs a{padding:9px 14px;border:1px solid var(--line);border-radius:10px;text-decoration:none;color:var(--d);font-weight:800;background:#fff}.marketTabs a.active{background:var(--b);border-color:var(--b);color:#fff}.marketTemplate{padding:16px;margin-bottom:14px}.marketTemplateHead strong{font-size:16px}.marketSavedBadge{color:#047857;background:#ecfdf5;border:1px solid #a7f3d0;border-radius:999px;padding:5px 9px;font-size:11px;font-weight:900}.marketForms{display:grid;grid-template-columns:1.25fr .8fr .6fr auto;gap:8px;margin-top:12px;align-items:end}.marketForms label{font-size:11px;font-weight:800;color:var(--mut)}.marketForms input,.marketForms select{width:100%;margin-top:4px}.marketFormula{padding:11px 14px;background:#eff6ff;border:1px solid #bfdbfe;color:#1e40af;border-radius:12px;font-weight:800;font-size:13px;margin-bottom:12px}.marketStatus{min-height:22px;color:#64748b;font-size:12px;font-weight:800}.marketStatus.saving{color:#b45309}.marketStatus.saved{color:#047857}.marketStatus.error{color:#b91c1c}.marketTableCard{overflow:hidden}.marketTableWrap{overflow-x:auto}.marketTable{width:100%;border-collapse:collapse;min-width:940px}.marketTable th{background:#f8fafc;color:#475569;font-size:12px;text-align:left;padding:11px 9px;border-bottom:1px solid var(--line)}.marketTable td{padding:8px 9px;border-bottom:1px solid #edf1f7;vertical-align:middle}.marketTable input,.marketTable select{width:100%;min-width:80px;height:38px;border:1.5px solid #cbd5e1;border-radius:9px;padding:7px 9px;font:inherit;background:#fff}.marketTable .marketName{min-width:210px}.marketCalculated{background:#ecfdf5;color:#047857;font-weight:900;border-radius:9px;padding:9px;min-height:38px}.marketDelete{width:38px;height:38px;min-height:38px!important;padding:0!important;background:#fff1f2!important;color:#be123c!important;border:1px solid #fecdd3!important;box-shadow:none!important}.marketResultBox{margin-top:14px;padding:15px}.marketResultValues{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:10px}.marketResultValue{background:#f8fafc;border:1px solid var(--line);border-radius:12px;padding:12px}.marketResultValue strong{display:block;font-size:22px;color:#1d4ed8}.marketHistoryGrid{display:grid;gap:10px;margin-top:14px}.marketHistoryCard{padding:16px;display:grid;grid-template-columns:minmax(0,1fr) auto;gap:12px;align-items:center}.marketHistoryMeta{display:flex;gap:12px;flex-wrap:wrap;color:var(--mut);font-size:12px;margin-top:5px}.marketHistoryActions{display:flex;gap:7px;flex-wrap:wrap}.marketHistoryActions form{margin:0}.marketPrintMenu{display:flex;gap:8px;flex-wrap:wrap}.marketEmpty{padding:34px;text-align:center;color:var(--mut)}
@media(max-width:760px){.marketTop,.marketTemplateHead,.marketHistoryHead{align-items:stretch;display:grid}.marketTopActions,.marketPrintActions{display:grid;grid-template-columns:1fr 1fr}.marketTopActions>*{width:100%}.marketTabs{overflow-x:auto;flex-wrap:nowrap;padding-bottom:4px}.marketTabs a{white-space:nowrap}.marketForms{grid-template-columns:1fr 1fr}.marketForms label:first-child{grid-column:1/-1}.marketForms button{grid-column:1/-1;width:100%}.marketTable{min-width:0}.marketTable thead{display:none}.marketTable,.marketTable tbody,.marketTable tr,.marketTable td{display:block;width:100%}.marketTable tr{padding:12px;border-bottom:1px solid var(--line)}.marketTable td{border:0;padding:5px 0}.marketTable td:before{content:attr(data-label);display:block;font-size:10px;color:var(--mut);font-weight:800;margin-bottom:3px}.marketTable .marketName{min-width:0}.marketDelete{width:100%}.marketResultValues{grid-template-columns:1fr}.marketHistoryCard{grid-template-columns:1fr}.marketHistoryActions{display:grid;grid-template-columns:1fr 1fr}.marketHistoryActions>*{width:100%}.marketHistoryActions .btn,.marketHistoryActions button{width:100%}}
</style>`;}
...
window.marketToggleResult=button=>{const box=document.getElementById('marketResultValues'),show=box.hidden;box.hidden=!show;button.textContent=show?'Приховати результат':'Показати результат'};
window.marketExport=(path,base,newTab)=>{const mode=document.getElementById('marketExportMode').value,scope=document.getElementById('marketExportScope').value;marketFlush().then(()=>{const url=path+'?'+base+'&mode='+encodeURIComponent(mode)+'&scope='+encodeURIComponent(scope);if(newTab)window.open(url,'_blank');else location.href=url}).catch(()=>{})};
})();
</script>`;}
function marketHistoryPage(db,url){
  ...
  if(record){
    ...
    const tabs=(record.sellerSnapshots||[]).map(s=>`<a class="${String(s.id)===sid?'active':''}" href="/admin-market/history?id=${encodeURIComponent(record.id)}&seller=${encodeURIComponent(s.id)}">${esc(s.name)}</a>`).join('');
    const rows=(record.products||[]).map(p=>{const row=record.rows&&record.rows[sid]&&record.rows[sid][p.id]||{},calc=marketCalc(record,sid,p.id);return `<tr><td data-label="Назва товару">${esc(p.name)}</td><td data-label="Ціна">${esc(marketMoney(p.priceCents))} / ${esc(p.unit)}</td><td data-label="Прийняв">${esc(marketQuantity(row.accepted,p.unit))}</td><td data-label="Здав">${esc(marketQuantity(row.returned,p.unit))}</td><td data-label="Продано">${calc.sold===null?'':esc(marketQuantity(calc.sold,p.unit))}</td><td data-label="Сума">${calc.sumCents===null?'':esc(marketMoney(calc.sumCents))}</td></tr>`;}).join('');
    const base=`marketId=${encodeURIComponent(record.id)}&seller=${encodeURIComponent(sid)}`;
    return `<div class="adminShell">${adminMenu('market')}<section class="marketPage">${marketStyles()}<div class="marketHistoryHead"><div><a href="/admin-market/history">← Історія ринків</a><h1>Ринок за ${esc(marketDateLabel(record.date))}</h1></div><div class="marketPrintMenu"><select id="marketHistoryMode"><option value="names">Лише назви товарів і ціни</option><option value="full">Повна таблиця</option></select><select id="marketHistoryScope"><option value="seller">Поточний продавець</option><option value="all">Усі продавці</option></select><button type="button" class="secondary" onclick="marketHistoryExport('/admin-market/print',true)">Друк</button><button type="button" onclick="marketHistoryExport('/admin-market/pdf',false)">PDF</button></div></div><div class="marketTabs">${tabs}</div><div class="card marketTableCard"><div class="marketTableWrap"><table class="marketTable"><thead><tr><th>Назва товару</th><th>Ціна</th><th>Прийняв</th><th>Здав</th><th>Продано</th><th>Сума</th></tr></thead><tbody>${rows}</tbody></table></div></div><div class="card marketResultBox"><button type="button" class="secondary" onclick="marketToggleResult(this)">Показати результат</button><div id="marketResultValues" class="marketResultValues" hidden><div class="marketResultValue"><small>Сума продавця ${esc(seller?seller.name:'')}</small><strong>${esc(marketMoney(marketSellerTotal(record,sid)))}</strong></div><div class="marketResultValue"><small>Загальна сума</small><strong>${esc(marketMoney(marketGrandTotal(record)))}</strong></div></div></div><script>function marketToggleResult(button){const box=document.getElementById('marketResultValues'),show=box.hidden;box.hidden=!show;button.textContent=show?'Приховати результат':'Показати результат'}function marketHistoryExport(path,newTab){const mode=document.getElementById('marketHistoryMode').value,scope=document.getElementById('marketHistoryScope').value,url=path+'?${base}&mode='+encodeURIComponent(mode)+'&scope='+encodeURIComponent(scope);if(newTab)window.open(url,'_blank');else location.href=url}</script></section></div>`;
  }
  const status=String(url.searchParams.get('status')||''),notice=status==='deleted'?'<div class="successMsg">Історію ринку видалено.</div>':'';
  const cards=state.history.slice().reverse().map(record=>`<div class="card marketHistoryCard"><div><h2>Ринок за ${esc(marketDateLabel(record.date))}</h2><div class="marketHistoryMeta"><span>${(record.sellerSnapshots||[]).length} продавців</span><span>${(record.products||[]).length} позицій</span><span>Загальна сума: ${esc(marketMoney(marketGrandTotal(record)))}</span></div></div><div class="marketHistoryActions"><a class="btn" href="/admin-market/history?id=${encodeURIComponent(record.id)}">Переглянути</a><a class="btn secondary" target="_blank" href="/admin-market/print?marketId=${encodeURIComponent(record.id)}&mode=full&scope=all">Друк</a><a class="btn secondary" href="/admin-market/pdf?marketId=${encodeURIComponent(record.id)}&mode=full&scope=all">PDF</a><form method="post" action="/admin-market/history-delete" onsubmit="return confirm('Видалити історію ринку за ${esc(marketDateLabel(record.date))}? Цю дію неможливо скасувати.')"><input type="hidden" name="id" value="${esc(record.id)}"><button class="danger">Видалити історію</button></form></div></div>`).join('');
...
function marketPrintPage(record,{mode='full',scope='seller',sellerId='',auto=false}={}){
  ...
  return `<!doctype html><html lang="uk"><head><meta charset="utf-8"><title>Ринок ${esc(marketDateLabel(record.date))}</title><style>@page{size:A4 ${mode==='full'?'landscape':'portrait'};margin:12mm}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#111;margin:0}.tools{position:fixed;right:12px;top:12px}.tools button{padding:10px 16px}h1{margin:0 0 4px}header{margin-bottom:18px}header p{margin:0;color:#555}.seller{page-break-after:always}.seller:last-child{page-break-after:auto}table{width:100%;border-collapse:collapse;font-size:11px}th,td{border:1px solid #777;padding:6px;text-align:left}th{background:#eef3f8}tfoot td{font-weight:bold;background:#f6f8fb}@media print{.tools{display:none}}</style></head><body><div class="tools"><button onclick="window.print()">Друк / зберегти PDF</button></div><header><h1>Ринок</h1><p>Дата: ${esc(marketDateLabel(record.date))}</p></header>${sections||'<p>Продавців немає.</p>'}${auto?'<script>setTimeout(()=>window.print(),250)</script>':''}</body></html>`;
}
...
<script>(function(){
  ...
  if(req.method==='POST' && url.pathname==='/admin-market/update'){
    if(!requireAdmin(req,res,session))return;const b=await body(req),state=marketState(db),record=state.active;
    if(!record||String(record.id)!==String(b.marketId||''))return sendJson(res,409,{ok:false,conflict:true,error:'Активний ринок змінився. Оновіть сторінку.'});
    ...
    if(['accepted','returned'].includes(field)){
      ...
      const template=state.template.find(p=>String(p.id)===productId);
    ...
    return sendJson(res,200,{ok:true,fieldVersion,normalized,sold:calc.sold===null?'':marketQuantity(calc.sold,product.unit)+' '+product.unit,sum:calc.sumCents===null?'':marketMoney(calc.sumCents),sellerTotal:marketMoney(marketSellerTotal(record,sellerId)),grandTotal:marketMoney(marketGrandTotal(record))});
  }
  ...
  if(req.method==='GET' && url.pathname==='/admin-market/print'){
    ...
    return send(res,marketPrintPage(record,{mode:url.searchParams.get('mode'),scope:url.searchParams.get('scope'),sellerId:url.searchParams.get('seller'),auto:url.searchParams.get('auto')==='1'}));
  }
  if(req.method==='GET' && url.pathname==='/admin-market/pdf'){
    if(!requireAdmin(req,res,session))return;const state=marketState(db),record=marketRecord(state,String(url.searchParams.get('marketId')||''));if(!record)return notFound(res);
    try{const pdf=await marketPdf(record,{mode:url.searchParams.get('mode'),scope:url.searchParams.get('scope'),sellerId:url.searchParams.get('seller')}),filename=`market_${String(record.date||'').replace(/[^0-9-]/g,'_')}.pdf`;res.writeHead(200,{'Content-Type':'application/pdf','Content-Disposition':`attachment; filename="${filename}"`,'Content-Length':pdf.length,'Cache-Control':'no-store','X-Content-Type-Options':'nosniff'});return res.end(pdf);}catch(error){console.error('[market-pdf]',error);return send(res,marketPrintPage(record,{mode:url.searchParams.get('mode'),scope:url.searchParams.get('scope'),sellerId:url.searchParams.get('seller')}),500);}
  }
